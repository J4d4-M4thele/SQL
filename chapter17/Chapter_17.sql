...
--table size doubled coz the numbers have been inserted
--into duplicate rows (so the table has 1 mill. rows)
UPDATE vacuum_test
...
--autovaccum runs in the background
SELECT relname,
       last_vacuum,
       last_autovacuum,
       vacuum_count,
...
pg_dump -t "temperature_test" -d analysis -U postgres -Fc > temperature_backup.sql 
