...
--listing 10.2(using correlation function to test relationships
--between columns)
SELECT corr(median_hh_income, pct_bachelors_higher)
    AS bachelors_income_r
FROM acs_2011_2015_stats;

--income increases as person is qualified by
--bachelors or higher

--listing 10.3(using corr(X,Y) on other values)
SELECT
round(
...
--bachelors income has weak corr. with travel 
--time and median income 
...
--rank() allows for gaps in ordering (provides for ties)
--dense_rank() doesn't, regardless of how many come before it 
...
SELECT
    ...
    store,
    unit_sales,
    --partition groups ranks by column specified
